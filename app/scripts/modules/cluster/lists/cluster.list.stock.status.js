/**
 * @ngdoc function
 * @name ngmReportHubApp.controller:ListStockStatusCtrl
 * @description
 * # ListStockStatusCtrl
 * Controller of the ngmReportHub
 */
angular.module('ngmReportHub')
    .controller('ListStockStatusCtrl', ['$scope', '$location', '$route', 'ngmAuth', 'ngmData', 'ngmUser', 'ngmClusterHelper', '$translate', '$filter', '$rootScope', 'ngmClusterLists', 'listService', function ($scope, $location, $route, ngmAuth, ngmData, ngmUser, ngmClusterHelper, $translate, $filter, $rootScope, ngmClusterLists, listService) {
        this.awesomeThings = [
            'HTML5 Boilerplate',
            'AngularJS',
            'Karma'
                ...
                $scope.model = {
                    ...
                    header: {
                        ...
                        title: {
                            ...
                            title: 'Stock Status | ' + $scope.list.title
                        },
                        subtitle: {
                            'class': 'col s12 m12 l12 report-subtitle hide-on-small-only',
                            title: 'List Stock Status For ' + $scope.list.title
                        },
                        download: {
                            ...
                            downloads: [
                                {
                                    ...
                                    icon: 'assignment',
                                    hover: 'Download Stock Status List CSV',
                                    // request: {
                                    //     method: 'GET',
                                    ...
                                    color: 'blue lighten-2',
                                    icon: 'description',
                                    hover: 'Download Stock Status List Excel',
                                    // request: {
                                    //     method: 'GET',
                    ...
                    },
                    {
                        columns: [{
                            styleClass: 's12 m12 l12',
                            widgets: [{
                                type: 'form.stock.status.list',
                                style: 'padding:0px; height: 90px; padding-top:10px;',
                                config: {
                                    style: $scope.list.ngm.style,
                                    stock_status: $scope.list.stock_status,
                                }
                            }]
                        }]
                    },
        ...
        ).then(function (stock_status) {
            if (stock_status.length) {
                cluster = $route.current.params.cluster_id ? $route.current.params.cluster_id : 'all';
                $scope.list.stock_status = stock_status;
                if (cluster !== 'all') {
                    $scope.list.stock_status = stock_status.filter(x => x.cluster_id === cluster);
                }
