/**
 * @ngdoc function
 * @name ngmReportHubApp.controller:ListBeneficiaryCategoriesCtrl
 * @description
 * # ListBeneficiaryCategoriesCtrl
 * Controller of the ngmReportHub
 */
angular.module('ngmReportHub')
    .controller('ListBeneficiaryCategoriesCtrl', ['$scope', '$location', '$route', 'ngmAuth', 'ngmData', 'ngmUser', 'ngmClusterHelper', '$translate', '$filter', '$rootScope', 'ngmClusterLists', 'listService', function ($scope, $location, $route, ngmAuth, ngmData, ngmUser, ngmClusterHelper, $translate, $filter, $rootScope, ngmClusterLists, listService) {
        this.awesomeThings = [
            'HTML5 Boilerplate',
            'AngularJS',
            'Karma'
                ...
                $scope.model = {
                    ...
                    header: {
                        ...
                        title: {
                            ...
                            title: 'Beneficiary Categories | ' + $scope.list.title
                        },
                        subtitle: {
                            'class': 'col s12 m12 l12 report-subtitle hide-on-small-only',
                            title: 'List Beneficiary Categories For ' + $scope.list.title
                        },
                        download: {
                            ...
                            downloads: [
                                {
                                    ...
                                    icon: 'assignment',
                                    hover: 'Download Beneficiary Categories List CSV',
                                    // request: {
                                    //     method: 'GET',
                                    ...
                                    color: 'blue lighten-2',
                                    icon: 'description',
                                    hover: 'Download Beneficiary Categories List Excel',
                                    // request: {
                                    //     method: 'GET',
                    ...
                    },
                    {
                        columns: [{
                            styleClass: 's12 m12 l12',
                            widgets: [{
                                type: 'form.beneficiary.categories.list',
                                style: 'padding:0px; height: 90px; padding-top:10px;',
                                config: {
                                    style: $scope.list.ngm.style,
                                    beneficiary_categories: $scope.list.beneficiary_categories,
                                }
                            }]
                        }]
                    },
        ...
        ).then(function (beneficiary_categories) {
            if (beneficiary_categories.length) {
                cluster = $route.current.params.cluster_id ? $route.current.params.cluster_id : 'all';
                $scope.list.beneficiary_categories = beneficiary_categories;
                if (cluster !== 'all') {
                    $scope.list.beneficiary_categories = beneficiary_categories.filter(x => x.cluster_id === cluster);
                }
