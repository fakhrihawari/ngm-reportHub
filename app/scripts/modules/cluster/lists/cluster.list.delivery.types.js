/**
 * @ngdoc function
 * @name ngmReportHubApp.controller:ListDeliveryTypesCtrl
 * @description
 * # ListDeliveryTypesCtrl
 * Controller of the ngmReportHub
 */
angular.module('ngmReportHub')
    .controller('ListDeliveryTypesCtrl', ['$scope', '$location', '$route', 'ngmAuth', 'ngmData', 'ngmUser', 'ngmClusterHelper', '$translate', '$filter', '$rootScope', 'ngmClusterLists', 'listService', function ($scope, $location, $route, ngmAuth, ngmData, ngmUser, ngmClusterHelper, $translate, $filter, $rootScope, ngmClusterLists, listService) {
        this.awesomeThings = [
            'HTML5 Boilerplate',
            'AngularJS',
            'Karma'
                ...
                $scope.model = {
                    ...
                    header: {
                        ...
                        title: {
                            ...
                            title: 'Delivery Types | ' + $scope.list.title
                        },
                        subtitle: {
                            'class': 'col s12 m12 l12 report-subtitle hide-on-small-only',
                            title: 'List Delivery Types For ' + $scope.list.title
                        },
                        download: {
                            ...
                            downloads: [
                                {
                                    ...
                                    icon: 'assignment',
                                    hover: 'Download Delivery Type List CSV',
                                    // request: {
                                    //     method: 'GET',
                                    ...
                                    color: 'blue lighten-2',
                                    icon: 'description',
                                    hover: 'Download Delivery Type List Excel',
                                    // request: {
                                    //     method: 'GET',
                    ...
                    },
                    {
                        columns: [{
                            styleClass: 's12 m12 l12',
                            widgets: [{
                                type: 'form.delivery.types.list',
                                style: 'padding:0px; height: 90px; padding-top:10px;',
                                config: {
                                    style: $scope.list.ngm.style,
                                    delivery_types: $scope.list.delivery_types,
                                }
                            }]
                        }]
                    },
        ...
        ).then(function (delivery_types) {
            if (delivery_types.length) {
                cluster = $route.current.params.cluster_id ? $route.current.params.cluster_id : 'all';
                $scope.list.delivery_types = delivery_types;
                if (cluster !== 'all') {
                    $scope.list.delivery_types = delivery_types.filter(x => x.cluster_id === cluster);
                }
