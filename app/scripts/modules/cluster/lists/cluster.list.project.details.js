/**
 * @ngdoc function
 * @name ngmReportHubApp.controller:ListProjectDetailCtrl
 * @description
 * # ListProjectDetailCtrl
 * Controller of the ngmReportHub
 */
angular.module('ngmReportHub')
    .controller('ListProjectDetailCtrl', ['$scope', '$location', '$route', 'ngmAuth', 'ngmData', 'ngmUser', 'ngmClusterHelper', '$translate', '$filter', '$rootScope', 'ngmClusterLists', 'listService', function ($scope, $location, $route, ngmAuth, ngmData, ngmUser, ngmClusterHelper, $translate, $filter, $rootScope, ngmClusterLists, listService) {
        this.awesomeThings = [
            'HTML5 Boilerplate',
            'AngularJS',
            'Karma'
                ...
                $scope.model = {
                    ...
                    header: {
                        ...
                        title: {
                            ...
                            title: 'Project Details | ' + $scope.list.title
                        },
                        subtitle: {
                    ...
                    },
                    {
                        columns: [{
                            styleClass: 's12 m12 l12',
                            widgets: [{
                                type: 'form.project.detail.list',
                                style: 'padding:0px; height: 90px; padding-top:10px;',
                                config: {
                                    style: $scope.list.ngm.style,
                                    project_details: $scope.list.project_details,
                                }
                            }]
                        }]
                    },
        ...
        ).then(function (project_details) {
            if (project_details.length) {
                cluster = $route.current.params.cluster_id ? $route.current.params.cluster_id : 'all';
                $scope.list.project_details = project_details;
                if (cluster !== 'all') {
                    $scope.list.project_details = project_details.filter(x => x.cluster_id === cluster);
                }
