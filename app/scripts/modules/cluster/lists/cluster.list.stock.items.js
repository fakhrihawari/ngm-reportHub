/**
 * @ngdoc function
 * @name ngmReportHubApp.controller:ListStockItemCtrl
 * @description
 * # ListStockItemCtrl
 * Controller of the ngmReportHub
 */
angular.module('ngmReportHub')
    .controller('ListStockItemCtrl', ['$scope', '$location', '$route', 'ngmAuth', 'ngmData', 'ngmUser', 'ngmClusterHelper', '$translate', '$filter', '$rootScope', 'ngmClusterLists', 'listService', function ($scope, $location, $route, ngmAuth, ngmData, ngmUser, ngmClusterHelper, $translate, $filter, $rootScope, ngmClusterLists, listService) {
        this.awesomeThings = [
            'HTML5 Boilerplate',
            'AngularJS',
            'Karma'
                ...
                $scope.model = {
                    ...
                    header: {
                        ...
                        title: {
                            ...
                            title: 'Stock Items | ' + $scope.list.title
                        },
                        subtitle: {
                            'class': 'col s12 m12 l12 report-subtitle hide-on-small-only',
                            title: 'List Stock Items For ' + $scope.list.title
                        },
                        download: {
                            ...
                            downloads: [
                                {
                                    ...
                                    icon: 'assignment',
                                    hover: 'Download Stock Item List CSV',
                                    // request: {
                                    //     method: 'GET',
                                    ...
                                    color: 'blue lighten-2',
                                    icon: 'description',
                                    hover: 'Download Stock Item List Excel',
                                    // request: {
                                    //     method: 'GET',
                    ...
                    },
                    {
                        columns: [{
                            styleClass: 's12 m12 l12',
                            widgets: [{
                                type: 'form.stock.item.list',
                                style: 'padding:0px; height: 90px; padding-top:10px;',
                                config: {
                                    style: $scope.list.ngm.style,
                                    stock_items: $scope.list.stock_items,
                                }
                            }]
                        }]
                    },
        ...
        ).then(function (stock_items) {
            if (stock_items.length) {
                cluster = $route.current.params.cluster_id ? $route.current.params.cluster_id : 'all';
                $scope.list.stock_items = stock_items;
                if (cluster !== 'all') {
                    $scope.list.stock_items = stock_items.filter(x => x.cluster_id === cluster);
                }
